...
class SettingsWindow(tk.Toplevel):
    def __init__(self, parent, app):
        ...
        self.lock_peace_btn = make_row(lp_icon, "Toggle Lock + Peace", app.toggle_lock_peace)
...
class TrackerApp(tk.Tk):
    ...
    def _build_toolbar(self):
        ...
        self.tb_moon_dark_photo = ImageTk.PhotoImage(
            resize_by_width(Image.open(resource_path("assets/Moon_Dark.png")).convert("RGBA"), ICON_SIZE)
        )
        # Lock + Peace button uses unlock.png (showing unlocked = icons visible) and lock.png (hidden)
        self.tb_unlock_photo = ImageTk.PhotoImage(
            resize_by_width(Image.open(resource_path("assets/unlock.png")).convert("RGBA"), ICON_SIZE)
        )
        self.tb_lock_photo = ImageTk.PhotoImage(
            resize_by_width(Image.open(resource_path("assets/lock.png")).convert("RGBA"), ICON_SIZE)
        )
